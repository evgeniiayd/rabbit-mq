...
Результат: ✅ Все города обработаны успешно
Время: ~3 секунды (5 потоков × 500ms задержка)
```

#### Тест 2: Обработка ошибок
```
Запрос: London, InvalidCity12345, Paris
Результат:
  - London: ✅ Успешно
  - InvalidCity12345: ❌ Ошибка → DLQ
  - Paris: ✅ Успешно
```
...
HTTP запросы к API: 3 (только уникальные города)
Кэш хиты: 3 (повторные запросы)
Hit Rate: 50%
```

#### Тест 4: Частичные результаты
```
Сценарий: Consumer Service остановлен
Запрос: London, Paris, Moscow
Результат через 30с:
  - Статус: ⚠️ Частичный результат
  - Получено: 0/3 ответов
  - Причина: "Timeout after 30s: received only 0/3 responses"
```

### 4.2 Производительность

| Сценарий | Города | Время обработки | HTTP запросы |
|----------|--------|----------------|--------------|
| Без оптимизаций | 10 | ~10 секунд | 10 |
| С масштабированием | 10 | ~2 секунды | 10 |
| С кэшированием (50% повторов) | 10 | ~1 секунда | 5 |

**Общий прирост производительности: 10x** 🚀
